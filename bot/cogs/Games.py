...
#                 if response == "rip":
#                     await ctx.channel.purge(limit=coins*2+2)
#                     await ctx.send(f"Ты победил {ctx.author.mention}. Я не знаю города да данную букву. Ты выиграл {coins*5} :leaves:")
#                     cursor.execute("UPDATE users SET cash = cash + {0} WHERE id = {1}".format(coins*5,ctx.author.id))
#                     refresh()
#                     return False
